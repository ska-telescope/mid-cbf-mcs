...
class TestFspSubarrayCorr:
    ...
    def test_AddRemoveReceptors_valid(
        ...
        """
        create_fsp_subarray_1_1_proxy.Init()
        ...
        create_fsp_subarray_1_1_proxy.AddReceptors([1, 10])
        assert create_fsp_subarray_1_1_proxy.receptors == (1, 10)

        # add more receptors
        create_fsp_subarray_1_1_proxy.AddReceptors([197])
        assert create_fsp_subarray_1_1_proxy.receptors == (1, 10, 197)

        # remove some receptors
        create_fsp_subarray_1_1_proxy.RemoveReceptors([10, 197])
        assert create_fsp_subarray_1_1_proxy.receptors == (1,)

        # remove remaining receptors
        create_fsp_subarray_1_1_proxy.RemoveReceptors([1])
        assert create_fsp_subarray_1_1_proxy.receptors == ()
    ...
    def test_AddRemoveReceptors_invalid(
        ...
        with pytest.raises(tango.DevFailed) as df:
            create_fsp_subarray_1_1_proxy.AddReceptors([17])
        assert "does not belong" in str(df.value.args[0].desc)
        assert create_fsp_subarray_1_1_proxy.receptors == (1, 10)

        # try adding an invalid receptor ID
        with pytest.raises(tango.DevFailed) as df:
            create_fsp_subarray_1_1_proxy.AddReceptors([200])
        time.sleep(1)
        ...
        create_fsp_subarray_1_1_proxy.RemoveReceptors([5])
        assert create_fsp_subarray_1_1_proxy.receptors == (1, 10)

        # remove all receptors
        create_fsp_subarray_1_1_proxy.RemoveReceptors([1, 10])
        assert create_fsp_subarray_1_1_proxy.receptors == ()
    ...
    def test_RemoveAllReceptors(
        ...
        # remove all receptors
        create_fsp_subarray_1_1_proxy.RemoveAllReceptors()
        assert create_fsp_subarray_1_1_proxy.receptors == ()
    ...
    def test_ConfigureScan_basic(
        ...
        create_fsp_subarray_1_1_proxy.ConfigureScan(f.read().replace("\n", ""))
        f.close()
