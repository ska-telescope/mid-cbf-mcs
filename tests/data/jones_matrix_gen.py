...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hp:n:t:", ["help", "path=", "name=", "tests="]
        )
    except getopt.GetoptError:
        print(
            "jones_matrix_gen.py -p <file path> -n <file name> -t <number of tests>"
        )
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-h", "--help"):
    ...
    for number_of_tests in range(
        num_tests
    ):  # number of receptor tests generated
        length = choice([4, 16])
        ...
        for number_of_receptors in [1, 3]:
            ...
            for fsid in range(1):
                ...
                for entry in range(
                    length
                ):  # number of entries in Jones matrix; 16 = 4x4 matrix
                    jones_matrix.append(
                        float(entry * (number_of_tests + 1))
                    )  # fill the matrix with known data
