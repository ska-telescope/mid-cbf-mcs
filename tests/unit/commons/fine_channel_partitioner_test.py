...
def test_partition_spectrum_to_frequency_slices_valid(parameters: dict):
    ...
    for _, fsp_config in results.items():
        ...
        assert (fsp_config["num_channels"]) % const.NUM_CHANNEL_GROUPS == 0
        assert (fsp_config["start_ch"]) % const.NUM_CHANNEL_GROUPS == 0
        assert (fsp_config["end_ch"] + 1) % const.NUM_CHANNEL_GROUPS == 0
