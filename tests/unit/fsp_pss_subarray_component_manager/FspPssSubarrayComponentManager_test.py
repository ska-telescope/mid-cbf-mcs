...
class TestFspPssSubarrayComponentManager:
    ...
    def test_configure_scan(
            ...
            assert fsp_pss_subarray_component_manager.fsp_id == configuration["fsp_id"]
            assert fsp_pss_subarray_component_manager.search_window_id == int(configuration["search_window_id"])
            for i, searchBeam in enumerate(configuration["search_beam"]):
                assert fsp_pss_subarray_component_manager.search_beams[i] == json.dumps(searchBeam)
                assert fsp_pss_subarray_component_manager.search_beam_id[i] == int(searchBeam["search_beam_id"])
