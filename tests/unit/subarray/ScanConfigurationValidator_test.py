...
from src.ska_mid_cbf_mcs.subarray.scan_configuration_validator import ScanConfigurationValidator
from src.ska_mid_cbf_mcs.subarray.subarray_component_manager import CbfSubarrayComponentManager
...
class TestScanConfigurationValidator:
    ...
    def before_each(self:TestScanConfigurationValidator,
    ...
    def test_Invalid_Subarray_ID(
            ...
            result_code, msg = validator.validate_input()
            print(msg)
            assert(f"subarray_id {subarray_id} not supported. MCS currently only supports [{1}]" in msg)
            assert(result_code == False)
    ...
    def test_Invalid_FSP_IDs(
            ...
            self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["fsp_ids"] = fsp_ids
            json_str = json.dumps(self.full_configuration)

            validator:ScanConfigurationValidator = ScanConfigurationValidator(json_str, subarray_component_manager,self.logger)
            result_code, msg = validator.validate_input()
            print(msg)
            assert(f"AA 0.5 only support 1-4 fsp_id with a single fsp_ids in a processing region, fsp_id given: {len(fsp_ids)}" in msg)
            assert(result_code == False)
    ...
    def test_Invalid_start_freq_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["start_freq"]=start_freq_value
        json_str = json.dumps(self.full_configuration)
    ...
    def test_Valid_start_freq_ADR99(
        ...
        for channel in range(0,channel_count_value-20+1,20):
            ...
            port += 1
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["start_freq"]=start_freq_value
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["channel_count"]=channel_count_value
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_port"] = output_ports_map
        json_str = json.dumps(self.full_configuration)
        print(self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_port"])
        validator:ScanConfigurationValidator = ScanConfigurationValidator(json_str, subarray_component_manager,self.logger)
        result_code, msg = validator.validate_input()
    ...
    @pytest.mark.parametrize(
            "channel_width",
            [13439,13441,210,420,840,1680,3360,6720,26880,40320,53760,80640,
    ...
    def test_Invalid_channel_width_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["channel_width"] = channel_width
        json_str = json.dumps(self.full_configuration)
    ...
    def test_Invalid_channel_count_ADR99(
        ...
        #Test cases to be added as more support channel widths are added       
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["channel_count"] = channel_count
        json_str = json.dumps(self.full_configuration)
    ...
    def test_Invalid_sdp_start_channel_id_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_host"][0][0] = 20
        ...
        with open(path_to_test_json) as file:
            json_str = file.read().replace("\n", "")
        self.full_configuration = json.loads(json_str)
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_port"][0][0] = 20
        json_str = json.dumps(self.full_configuration)
        ...
        with open(path_to_test_json) as file:
            json_str = file.read().replace("\n", "")
        self.full_configuration = json.loads(json_str)
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_link_map"][0][0] = 20
        json_str = json.dumps(self.full_configuration)
    ...
    @pytest.mark.parametrize(
              "output_host",
              [[[60, "1.22.3.4"],[40, "1.22.3.5"],[20, "1.22.3.6"],[0, "1.22.3.7"]],
               [[0, "1.22.3.4"],[40, "1.22.3.5"],[40, "1.22.3.6"],[60, "1.22.3.7"]],
               [[20, "1.22.3.4"],[21, "1.22.3.5"],[22, "1.22.3.6"],[42, "1.22.3.7"]],]
    )
    def test_Invalid_output_host_increment_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["sdp_start_channel_id"] = output_host[0][0]
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_host"] = output_host
        json_str = json.dumps(self.full_configuration)
    ...
    def test_Invalid_output_port_increment_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["sdp_start_channel_id"] = output_port[0][0]
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_host"] = output_port
        json_str = json.dumps(self.full_configuration)
    ...
    def test_Valid_channel_map_increment_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["sdp_start_channel_id"] = 1
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["channel_count"] = 80
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_port"] = [[1, 10000],[21, 10001],[41, 1650],[61, 40000]]
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_host"] = [[1, 10000],[21, 10001],[41, 1650],[61, 40000]]
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_link_map"] = [[1, 10000]]
        json_str = json.dumps(self.full_configuration)
    ...
    def test_invalid_channel_map_count_to_single_host_ADR99(
        ...
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_port"]
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["output_port"] = test_output_port_map
        json_str = json.dumps(self.full_configuration)
    ...
    def test_invalid_more_channel_in_channel_maps_than_channel_count(
        ...
        channel_count = self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["channel_count"]
        self.full_configuration["midcbf"]["correlation"]["processing_regions"][0]["channel_count"] = channel_count - 20
        json_str = json.dumps(self.full_configuration)
