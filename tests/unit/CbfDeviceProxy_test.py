...
class TestCbfDeviceProxy:
    ...
    def test_subscription(
        ...
        callback on an CbfDeviceProxy, this results in the underlying
        ...
        """
        fqdn = "mock/mock/1"
        mccs_device_proxy = CbfDeviceProxy(fqdn, logger)
    ...
    def test_event_pushing(
        ...
        device_proxy = CbfDeviceProxy(fqdn, logger)
            ...
            for j in range(event_count):
                if i == j:
                    mock_callbacks[j].assert_called_once_with(
                        mock_event, event_name, event_value, event_quality
                    )
                    mock_callbacks[j].reset_mock()
                else:
                    mock_callbacks[j].assert_not_called()
