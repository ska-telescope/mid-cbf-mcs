...
LOG_FORMAT = "[THREAD: %(threadName)s]->[bite.py: line %(lineno)s]%(levelname)s: %(message)s"
...
class ECBite(SKABaseDevice):
    ...
    test_param_location = attribute(
        label="Test Parameters File Folder",
        dtype=str,
        access=AttrWriteType.READ_WRITE,
        doc="The directory generated by cbf system tests to pull from for test data.",
        fget="get_test_param_location",
        fset="set_test_param_location",
    )

    # -- HELPER FUNCTIONS --
    def load_in_input_json(self):
        """
        Helper function that reads in test config data.
        Uses the _test_param_location var in the TANGO object to find these files
        ...
        logger_.info("Setting test param location to -> %s", TEST_PARAMS_DIR)
        CBF_INPUT_DATA_DIR = os.path.join(TEST_PARAMS_DIR, "cbf_input_data")
        BITE_CONFIGS_DIR = os.path.join(
            CBF_INPUT_DATA_DIR, "bite_config_parameters"
        )
        ...
        self.cbf_input_data_path = os.path.join(
            CBF_INPUT_DATA_DIR, "cbf_input_data.json"
        )
        self.test_data_path = os.path.join(TEST_PARAMS_DIR, "tests.json")
        self.bite_configs_path = os.path.join(
            BITE_CONFIGS_DIR, "bite_configs.json"
        )
        self.filters_path = os.path.join(BITE_CONFIGS_DIR, "filters.json")

        # In case the proper test files aren't loaded, we use the in-device fallback file to load basic params
        ...
        if (
            ...
            self.test_data_path = BASIC_TEST_PARAMETERS
        ...
        with open(self.cbf_input_data_path, encoding="utf-8") as f:
    ...
    def init_device(self):
        logger_.info("Start TANGO device init...")
    ...
    def get_test_param_location(self):
        """TANGO function that returns the currently set directory for test data"""
        return self._test_param_location

    def set_test_param_location(self, test_param_location):
        """TANGO function that sets the directory to access tests data from, and reads in the data from it"""
        self._test_param_location = test_param_location
        return self.load_in_input_json()
