...
class ScanConfigurationValidator:
    ...
    def validate_input(self: ScanConfigurationValidator) -> tuple[bool, str]:
        ...
        if scan_configuration_version in ScanConfiguration.ADR99_VERSIONS:
            ...
            result_code, msg = self._validate_input_ADR99(configuration,
                                                          common_configuration)
        
        # Pre ADR99 Scan Configuration Changes 
        elif scan_configuration_version in ScanConfiguration.PRE_ADR99_VERSIONS:
            configuration = copy.deepcopy(full_configuration["cbf"])
            result_code, msg = self._validate_input_pre_ADR99(configuration,
                                                         common_configuration)
    ...
    def _validate_input_pre_ADR99(
        ...
        """
        Validates if the Scan Configuration in self.scan_configuration is valid with pre 4.0 versions
    ...
    def _validate_input_ADR99(
        ...
        """
        Validates if the Scan Configuration in self.scan_configuration is valid for pre ADR 99 versions
        ...
        for fsp in configuration["fsp"]:
            ...
            try:
                ...
                match function_mode_value:
                    case 1:
                        result_code, msg = self.validate_corr_function_mode(fsp, function_mode_value)

                    case 2:
                        result_code, msg = self.validate_pss_function_mode(fsp, function_mode_value)
                    
                    case 3:
                        result_code, msg = self.validate_pst_function_mode(fsp, function_mode_value)
    ...
    def _validate_fsp_adr_99(self: ScanConfigurationValidator,
                            configuration: dict,
                            common_configuration: dict) -> tuple[bool, str]:
        ...
        if "correlation" in configuration:
            ...
            self._validate_fsp_function_mode_adr_99("correlation",configuration,common_configuration,self.validate_corr_function_mode)
    
        if "pss" in configuration:
            # fsp = group of processing regions.  Variable was named fsp for compatibility with abstracted functions for 2.4 validations 
            self._validate_fsp_function_mode_adr_99("correlation",configuration,common_configuration,self.validate_pss_function_mode)

        if "pst" in configuration:
            # fsp = group of processing regions.  Variable was named fsp for compatibility with abstracted functions for 2.4 validations 
            self._validate_fsp_function_mode_adr_99("correlation",configuration,common_configuration,self.validate_pst_function_mode)

        msg = "Validate FSP: Validation Complete"
        self.logger.info(msg)
        return (True, msg)

    def _validate_fsp_function_mode_adr_99(self: ScanConfigurationValidator,
                                           fucntion_mode:str,
                                           configuration: dict,
                                           common_configuration:dict,
                                           mode_validate_func)-> tuple[bool, str]:
        for fsp in configuration[fucntion_mode]["processing_regions"]:
                fsp = copy.deepcopy(fsp)
                count = 0
                for fsp_id_str in fsp["fsp_ids"]:
                    try:
                ...
                #validate Corr for each processing regions
                result_code, msg = mode_validate_func(fsp,common_configuration)
                if result_code == False:
                    return (False, msg)
        
        msg = f"FSP Validation: Complete for {fucntion_mode} function mode"
        self.logger.info(msg)
        return (True, msg)
    ...
    def validate_pss_function_mode(self: ScanConfigurationValidator,
                                    fsp: dict,
                                    common_configuration: dict = None)-> tuple[bool, str]:
    ...
    def validate_vcc(self: ScanConfigurationValidator):
        for dish_id, proxy in self._proxies_assigned_vcc.items():
            if proxy.State() != tango.DevState.ON:
                msg = f"VCC {self._proxies_vcc.index(proxy) + 1} is not ON. Aborting configuration."
                return (False, msg)
    ...
    def validate_search_window(self: ScanConfigurationValidator,
        ...
        if "search_window" in configuration:
            ...
            msg = "Validate Search Window: Search Window not in Configuration"
            self.logger.info(msg)
            return (True, msg)
        
        msg = "Validate Search Window: Search Window Configuration is Valid"
        self.logger.info(msg)
        return (True, msg)
