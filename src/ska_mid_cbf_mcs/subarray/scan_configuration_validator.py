...
class ScanConfigurationValidator:
    ...
    def __init__(
        ...
        self._proxies_assigned_vcc = subarray_component_manager._proxies_assigned_vcc
        self._proxies_fsp_pss_subarray_device = subarray_component_manager._proxies_fsp_pss_subarray_device
    ...
    def validate_input(self: ScanConfigurationValidator) -> tuple[bool, str]:
        ...
        # Pre ADR99 Scan Configuration Changes 
            ...
            result_code, msg = self._validate_input_pre_adr_99(configuration,
    ...
    def _validate_input_pre_adr_99(
        ...
        result_code, msg = self._validate_fsp_pre_adr_99(configuration,common_configuration)
        if result_code == False:
            return (False, msg)
        
        return (True, "Scan configuration is valid.")
 
    def _validate_fsp_pre_adr_99(self: ScanConfigurationValidator,
                                 configuration: dict,
    ...
    def _validate_fsp_id(self: ScanConfigurationValidator, 
                         fsp_id: int)-> tuple[bool, str]:
        ...
        if fsp_id > 4:
            msg = (f"AA 0.5 Requirment: Supports only FSP 1-4."
                    " FSP ID given: {fsp_id}")
            self.logger.error(msg)
            return (False, msg)
    ...
    def _validate_input_adr_99(
        ...
        if "pss" in full_configuration:
            result_code, msg = self._validate_pss_function_mode_adr_99(full_configuration["pss"])
            if result_code == False:
                return (False, msg)
        
        if "pst" in full_configuration:
            result_code, msg =self._validate_pst_function_mode_adr_99(full_configuration["pst"])
            if result_code == False:
                return (False, msg)
    ...
    def _validate_midcbf(self: ScanConfigurationValidator,
                            configuration: dict) -> tuple[bool, str]:
        """
        ...
        if "correlation" in configuration:
            ...
            function_mode_value = FspModes[self.adr_99_function_mode_match["correlation"]]
            result_code, msg = self._validate_processing_regions("correlation",function_mode_value,configuration)
            if result_code == False:
                return (False, msg)
    ...
    def _validate_processing_regions(self: ScanConfigurationValidator,
                                    function_mode:str,
                                    function_mode_value: int,
                                    configuration: dict)-> tuple[bool, str]:
        """
        ...
        seen_fsp_id = set()
        for processing_region in configuration[function_mode]["processing_regions"]:
            processing_region = copy.deepcopy(processing_region)

            if len(processing_region["fsp_ids"]) > 4 or len(processing_region["fsp_ids"]) < 1:
            ...
            result_code, msg = self._validate_channels_maps(output_host,"output_host",sdp_start_channel_id,channel_count)
            if result_code == False:
                return (False, msg)

            output_port = processing_region["output_port"]
            result_code, msg = self._validate_channels_maps(output_port,"output_port",sdp_start_channel_id,channel_count)
            if result_code == False:
                return (False, msg)
            
            output_link_map = processing_region["output_link_map"]
            result_code, msg = self._validate_channels_maps(output_link_map,"output_link_map",sdp_start_channel_id,channel_count)
            if result_code == False:
                return (False, msg)                
            
            
            result_code, msg =self._validate_max_20_channel_to_same_port_per_host(output_host,output_port,sdp_start_channel_id,channel_count)
            if result_code == False:
                return (False, msg)     
            ...
            for fsp_id_str in processing_region["fsp_ids"]:
                try:
                    ...
                    result_code, msg = self._validate_fsp_id_adr_99(fsp_id,FspModes(function_mode_value),seen_fsp_id)
                    if result_code == False:
                        return (False, msg)
                    count += 1
                except KeyError:
            ...
            result_code, msg = self._validate_corr_function_mode_adr_99(processing_region)
            if result_code == False:
                return (False, msg)
    ...
    def _validate_corr_function_mode_adr_99(self: ScanConfigurationValidator,
    ...
    def _validate_pst_function_mode_adr_99(self:ScanConfigurationValidator,
                                           pst_configuration: dict)->tuple[bool, str]:
        return (True, "Place Holder Function")        

    def _validate_pss_function_mode_adr_99(self:ScanConfigurationValidator,
    ...
    def _validate_processing_region_frequency(self: ScanConfigurationValidator,
        ...
        result_code, msg = self._validate_processing_region_within_bandwidth(processing_region)
        if result_code == False:
            return (False, msg)

        # Check that we have enough FSP to cover the required Bandwidth requested
        result_code, msg = self._validate_fsp_requirement_by_given_bandwidth(processing_region)
        if result_code == False:
            return (False, msg)
    ...
    def _validate_fsp_requirement_by_given_bandwidth(self:ScanConfigurationValidator,
        ...
        coarse_channel_low = math.floor((start_freq + const.FS_BW//2) / const.FS_BW)
        coarse_channel_high = math.floor((end_freq + const.FS_BW//2) / const.FS_BW)
        coarse_channels = list(range(coarse_channel_low, coarse_channel_high + 1))
        
        if len(fsp_given) < len(coarse_channels):
            msg = ("Not enought FSP Given in the Processing Region for the Frequency Band Specified in the Common"
                   f"\nFSP Required: {len(coarse_channels)} FSP Given: {len(fsp_given)}")
            self.logger.error(msg)
            return (False, msg)
        

        msg = ("Validate FSP requirement by Freqency Band: Complete:"
                f"FSP Required: {coarse_channels} FSP Given: {fsp_given}")
        self.logger.info(msg)
        return(True, msg)

    def _validate_processing_region_within_bandwidth(self:ScanConfigurationValidator,
                                               processing_region:dict
                                               ) -> tuple[bool,str]:
        """
        ...
        if (lower_freq < lower) or (upper_freq > upper):
                msg = ("The Processing Region is not within the range for the [0-1981808640] that is acepted by MCS",
        ...
        lower, upper = const.FREQUENCY_BAND_1_RANGE_HZ[0], const.FREQUENCY_BAND_2_RANGE_HZ[1]
        if (lower_freq < lower) or (upper_freq > upper):
                msg = (f"The Processing Region is not within the range for [Band1.lower-Band2.upper]: [{lower}-{upper}]",
                       f"\nProcessing Region range: {lower_freq} - {upper_freq} with starting center at {start_freq}")
    ...
    def _validate_channels_maps(self:ScanConfigurationValidator,
        ...
        if sdp_start_channel_id != map_pairs[0][0]:
            msg =(  f"Start Channel ID ({sdp_start_channel_id}) must be the same must match the" 
        ...
        for channel, value in map_pairs:
            if channel - prev != 20:
                msg = (f"{map_type} channel map pair [{channel},{value}]:" ,
    ...
    def _validate_max_20_channel_to_same_port_per_host(self: ScanConfigurationValidator,
                                                       output_host_map: dict,
                                                       output_port_map: dict,
                                                       sdp_start_channel_id: int,
                                                       channel_count: int)-> tuple[bool, str]:
        """
        ...
        for channel in range(sdp_start_channel_id, sdp_start_channel_id+channel_count):
            ...
            if host_port_channel_count[current_host][current_port] > 20:
                msg = ("There are over 20 channels assigned to a specific port within a single host "
    ...
    def _validate_fsp_id_adr_99(self: ScanConfigurationValidator, 
                         fsp_id: int,
                         fsp_mode: FspModes,
                         seen_fsp_id: set[int])-> tuple[bool, str]:
    ...
    def _validate_common(self:ScanConfigurationValidator,
                        common_configuration: dict) -> tuple[bool, str]:
        """
    ...
    def _validate_midcbf_keys(self: ScanConfigurationValidator,
                                    configuration: dict) -> tuple[bool, str]:
        """
