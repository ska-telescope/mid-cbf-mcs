...
def _find_fine_channel(
    ...
    last = None
    for n in range(
        0, const.NUM_FINE_CHANNELS_PR // const.NUM_CHANNELS_PER_SPEAD_STREAM
    ):  # == 0 to 744
        n2 = (
            -const.NUM_FINE_CHANNELS_PR // 2
...
def partition_spectrum_to_frequency_slices(
    ...
    for index, fs in enumerate(coarse_channels):
        ...
        fsp_info["fsp_start_ch"] = (
            fsp_info["start_ch"] + const.NUM_FINE_CHANNELS_PR // 2
        )
        fsp_info["fsp_end_ch"] = (
            fsp_info["end_ch"] + const.NUM_FINE_CHANNELS_PR // 2
        )
